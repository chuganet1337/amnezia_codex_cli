...
Репозиторий содержит два отдельных комплекта. Они запускают Codex CLI и все его
дочерние процессы в выделенном AmneziaWG network namespace. Остальной трафик
сервера продолжает идти через обычную сеть.
...
Ubuntu 26.04 `resolute` сейчас не поддерживается PPA Amnezia: в PPA нет каталога
`resolute`, поэтому `apt update` возвращает `404` и `does not have a Release file`.
Для автоматической установки используйте Ubuntu Server 24.04 LTS `noble`.
Исправление уже добавленного сломанного PPA описано в Ubuntu README.
